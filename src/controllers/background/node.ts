...
export default class NodeService extends GenericService {
  ...
  async verifyMessage(msg: string, signature: string, address: string) {
    ...
    const headers = await this.getHeaders();
    const result = await this.fetchWrapper(null, {
      method: "POST",
      headers: headers,
      body: JSON.stringify({
  ...
  async getBlockEntry(height: number) {
    ...
    const blockEntry = await this.fetchWrapper(`header/${height}`, {
      method: "GET",
      headers: headers,
    });
  ...
  async fetchWrapper(path: string | null, init: RequestInit): Promise<any> {
    const { apiHost } = await this.exec("setting", "getAPI");
